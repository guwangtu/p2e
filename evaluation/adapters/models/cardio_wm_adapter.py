...
@REGISTRY.register_model("cardio_wm")
class CardioWMAdapter(BaseModelAdapter):
    """Adapter for CardioWorldModel (LitCardioWorldModel) evaluation."""
    ...
    def setup(self) -> None:
        ...
        if not ckpt_path:
            raise ValueError("CardioWMAdapter requires 'ckpt_path' in config.")
        if not os.path.exists(ckpt_path):
            raise FileNotFoundError(f"Checkpoint not found: {ckpt_path}")

        self.model = LitCardioWorldModel.load_from_checkpoint(ckpt_path, map_location=self.device)
        self.model.to(self.device)
        self.model.eval()

    def predict(self, batch: EvalBatch) -> EvalPrediction:
        if self.model is None:
            raise RuntimeError("CardioWMAdapter.setup must be called before predict.")

        ppg_cycles = batch.ppg.to(self.device).float()    # (B, W, L)
        rr = batch.labels.get("rr_intervals", None)

        if rr is None:
            raise ValueError("CardioWMAdapter requires 'rr_intervals' in batch.labels")
        ...
        # Reshape to (B, W, L) to match ground truth from mimic_cycle adapter
        W = ppg_cycles.shape[1]
        L = ppg_cycles.shape[2]
        ecg_pred = ecg_pred.reshape(-1, W, L)
